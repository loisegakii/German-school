 ...
 *  1. Step 1 now collects phone number + country/region
 *  2. Phone number pre-fills the M-Pesa field in Step 3 automatically
 *  3. sendMpesa() calls real Daraja backend (POST /api/payments/mpesa/initiate/)
 *  4. Polls GET /api/payments/mpesa/status/:id every 3s — only advances when
...
const COUNTRIES = [
  ...
  'Ghana', 'South Africa', 'Germany', 'United Kingdom', 'United States',
...
function EmailToast({ email, onDone }) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '28px', right: '28px', zIndex: 9999, transform: visible && !exiting ? 'translateY(0) scale(1)' : 'translateY(100px) scale(0.95)', opacity: visible && !exiting ? 1 : 0, transition: 'all 0.5s cubic-bezier(0.16, 1, 0.3, 1)', pointerEvents: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px', background: 'rgba(6,9,15,0.92)', backdropFilter: 'blur(24px)', border: '1px solid rgba(16,185,129,0.4)', borderRadius: '18px', padding: '18px 22px', boxShadow: '0 24px 64px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.05)', minWidth: '320px', maxWidth: '380px' }}>
        <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'linear-gradient(135deg,#059669,#10B981)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 8px 24px rgba(16,185,129,0.45)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '13.5px', fontWeight: '700', color: '#fff', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '6px' }}>Welcome email sent! <span style={{ fontSize: '15px' }}>🎉</span></div>
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '12px', color: 'rgba(255,255,255,0.45)', lineHeight: '1.55' }}>Check your inbox at <span style={{ color: 'rgba(255,255,255,0.75)', fontWeight: '600', wordBreak: 'break-all' }}>{email}</span></div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <input type={isPassword?(show?'text':'password'):type} value={value} onChange={onChange} maxLength={maxLength} onFocus={()=>setFocused(true)} onBlur={()=>setFocused(false)} placeholder={placeholder}
...
function GoogleSignUpButton({ onSuccess, onError, loading }) {
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      } catch { onError('Failed to retrieve your Google profile. Please try again.') }
    },
  ...
  return (
    <button type="button" onClick={()=>googleLogin()} disabled={loading}
      style={{ width:'100%', padding:'13px', background:loading?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.14)', borderRadius:'12px', color:'#fff', fontFamily:"'DM Sans',sans-serif", fontSize:'14px', fontWeight:'600', cursor:loading?'not-allowed':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', marginBottom:'18px', transition:'all 0.22s', opacity:loading?0.65:1 }}
...
function PaymentStep({ level, currency, onCurrencyChange, onSuccess, onBack, prefillPhone }) {
  ...
  const [mpesa,      setMpesa]      = useState({ phone: prefillPhone || '' })
  const [card,       setCard]       = useState({ number: '', expiry: '', cvv: '', name: '' })
  const [mpesaState, setMpesaState] = useState('idle')   // idle | pushing | waiting | done | error
  const [cardState,  setCardState]  = useState('idle')   // idle | processing | done
  ...
  const sendMpesa = async () => {
    ...
    } else if (!/^(07|01|2547|2541)\d{8}$/.test(raw.replace('+', ''))) {
      e.phone = 'Enter a valid Safaricom number (07XX or 254XXXXXXXXX)'
    }
    ...
    try {
      ...
      countdownRef.current = setInterval(() => {
        ...
        if (t <= 0) {
          stopAll()
          setMpesaState('error')
          setMpesaError('No response from M-Pesa. The prompt may have expired — please try again.')
        }
      }, 1000)
      ...
      // Hard stop after 90s (Daraja prompt expires ~60s)
      timeoutRef.current = setTimeout(() => {
        if (pollRef.current) {
          stopAll()
          setMpesaState('error')
          setMpesaError('M-Pesa request timed out. Please try again.')
        }
      }, 90000)

    } catch (err) {
      setMpesaState('error')
      setMpesaError(err.message || 'Could not initiate M-Pesa payment. Please check your connection.')
    }
  }
        ...
        <div style={{ marginBottom:'20px' }}>
          ...
          {mpesaState === 'idle' && (
            <>
              ...
              <InputField label="" type="tel" value={mpesa.phone} onChange={e=>{setMpesa({phone:e.target.value});setErrors({})}} placeholder="07XX XXX XXX or 254XXXXXXXXX" icon={<Phone size={15} strokeWidth={2}/>} error={errors.phone} hint="We'll send an M-Pesa PIN prompt to this number"/>
              <div style={{ marginTop:'14px', display:'flex', flexDirection:'column', gap:'8px' }}>
                {[{n:'1',text:'Confirm your Safaricom number above'},{n:'2',text:"Click \"Send STK Push\" — a PIN prompt appears on your phone"},{n:'3',text:'Enter your M-Pesa PIN on your phone to confirm payment'}].map(s=>(
          ...
          {mpesaState === 'pushing' && (
            <div style={{ textAlign:'center', padding:'28px 0' }}>
              <div style={{ width:'56px', height:'56px', borderRadius:'50%', background:'rgba(0,166,81,0.1)', border:'2px solid rgba(0,166,81,0.3)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
                <div style={{ width:'24px', height:'24px', border:'2.5px solid rgba(0,166,81,0.3)', borderTop:'2.5px solid #00A651', borderRadius:'50%', animation:'spin 0.8s linear infinite' }}/>
              </div>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'18px', fontWeight:'700', color:'#fff', marginBottom:'6px' }}>Sending STK Push…</div>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'rgba(255,255,255,0.38)' }}>Contacting Safaricom — this takes a moment</div>
            </div>
          )}
          ...
          {mpesaState === 'waiting' && (
            <div style={{ textAlign:'center', padding:'24px 0' }}>
              ...
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'20px', fontWeight:'700', color:'#fff', marginBottom:'8px' }}>Check your phone now</div>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'rgba(255,255,255,0.45)', lineHeight:'1.75', maxWidth:'300px', margin:'0 auto 18px' }}>
                An M-Pesa prompt has appeared on <strong style={{ color:'#fff' }}>{mpesa.phone}</strong>.{' '}
                <strong style={{ color:'#10B981' }}>Enter your M-Pesa PIN</strong> to confirm <strong style={{ color:'#fff' }}>{displayPrice}</strong>.
              </div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'8px 18px', background:'rgba(0,166,81,0.08)', border:'1px solid rgba(0,166,81,0.2)', borderRadius:'100px', marginBottom:'14px' }}>
                <div style={{ width:'8px', height:'8px', borderRadius:'50%', background:'#00A651', animation:'ping 1s ease-in-out infinite' }}/>
                <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'#00A651', fontWeight:'600' }}>Waiting for PIN · {timer}s</span>
              </div>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'12px', color:'rgba(255,255,255,0.25)' }}>
                Do not close this page. It updates automatically once your PIN is confirmed.
              </div>
            </div>
          )}
          ...
          {mpesaState === 'done' && (
            <div style={{ textAlign:'center', padding:'20px 0' }}>
              ...
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'18px', fontWeight:'700', color:'#10B981', marginBottom:'6px' }}>Payment Confirmed</div>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'rgba(255,255,255,0.38)' }}>M-Pesa PIN accepted — activating your course…</div>
            </div>
          )}
          ...
          {mpesaState === 'error' && (
            <div style={{ textAlign:'center', padding:'20px 0' }}>
              ...
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'17px', fontWeight:'700', color:'#EF4444', marginBottom:'8px' }}>Payment Unsuccessful</div>
              <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'13px', color:'rgba(255,255,255,0.45)', lineHeight:'1.7', maxWidth:'300px', margin:'0 auto 20px' }}>{mpesaError}</div>
              <button onClick={resetMpesa}
                style={{ padding:'11px 28px', borderRadius:'10px', border:'1px solid rgba(0,166,81,0.4)', background:'rgba(0,166,81,0.08)', color:'#00A651', fontFamily:"'DM Sans',sans-serif", fontSize:'14px', fontWeight:'700', cursor:'pointer', transition:'all 0.2s' }}
                onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,166,81,0.16)'}}
...
export default function Register() {
  ...
  const handleGoogleSuccess = async ({ tokenResponse, userInfo }) => {
    ...
    try {
      ...
      onRegistered(data, { firstName: userInfo.given_name ?? '', lastName: userInfo.family_name ?? '', cefrLevel: form.cefrLevel, goal: form.goal || 'general', isGoogle: true })
    } catch (err) {
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (form.phone) {
        ...
        if (!/^(07|01|\+?2547|\+?2541)\d{8}$/.test(raw)) e.phone = 'Enter a valid phone number'
      }
        ...
        <div style={{ position:'relative', background:'linear-gradient(145deg,#06090F 0%,#0D1B35 60%,#06090F 100%)', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'48px', overflow:'hidden' }}>
          ...
          <div style={{ position:'relative', zIndex:1 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
              {[{level:'A1',label:'Complete Beginner',usd:0,note:'7-day free'},{level:'A2',label:'Elementary',usd:300},{level:'B1',label:'Intermediate',usd:350},{level:'B2',label:'Upper-Intermediate',usd:400},{level:'C1',label:'Advanced',usd:450},{level:'C2',label:'Mastery',usd:500}].map(p=>(
          ...
          <div className="fade-up" style={{ width:'100%', maxWidth:'448px' }}>
            ...
            {step === 1 && (
              <div className="fade-up">
                ...
                <GoogleSignUpButton onSuccess={handleGoogleSuccess} onError={msg=>setApiError(msg)} loading={googleLoad}/>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'13px' }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:'7px' }}>
                    ...
                    <div style={{ position:'relative' }}>
                      <span style={{ position:'absolute', left:'14px', top:'50%', transform:'translateY(-50%)', display:'flex', alignItems:'center', opacity:0.4 }}><Globe size={15} strokeWidth={2}/></span>
                      <select value={form.country} onChange={e=>update('country',e.target.value)}
                        style={{ width:'100%', padding:'13px 40px 13px 44px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'12px', color:form.country?'#fff':'rgba(255,255,255,0.25)', fontFamily:"'DM Sans',sans-serif", fontSize:'14px', outline:'none', cursor:'pointer', transition:'all 0.25s' }}>
                  ...
                  {form.cefrLevel && pricing && (
                    <div style={{ marginTop:'8px', padding:'10px 14px', background:pricing.usdPrice===0?'rgba(16,185,129,0.07)':'rgba(59,130,246,0.07)', border:`1px solid ${pricing.usdPrice===0?'rgba(16,185,129,0.22)':'rgba(59,130,246,0.22)'}`, borderRadius:'9px' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        ...
                        <span style={{ fontFamily:"'Playfair Display',serif", fontSize:'18px', fontWeight:'800', color:pricing.usdPrice===0?'#10B981':'#fff' }}>{pricing.usdPrice===0?'Free':formatPrice(pricing.usdPrice,currency)}</span>
                      </div>
                      {pricing.usdPrice>0&&<div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'11px', color:'rgba(255,255,255,0.28)', marginTop:'3px', textAlign:'right' }}>{currency==='USD'?`≈ KES ${getRawKes(pricing.usdPrice).toLocaleString()}`:`≈ $${pricing.usdPrice}`}</div>}
                      {pricing.usdPrice===0&&pricing.usdAfterTrial&&<div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:'11px', color:'rgba(255,255,255,0.35)', marginTop:'4px' }}>After 7 days: {formatPrice(pricing.usdAfterTrial,currency)}/module</div>}
                    </div>
                  )}
                </div>
