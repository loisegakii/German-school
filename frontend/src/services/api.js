...
const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL || 'https://german-school-mlnl.onrender.com/api').replace(/\/$/, '')
  ...
  async err => {
    ...
    if (err.response?.status === 401 && !original._retry) {
      ...
      try {
        ...
        const { data } = await axios.post(REFRESH_ENDPOINT, { refresh })
